...
class RpcContextCreator {
    ...
    create(instance, callback, module, methodName, contextId = constants_3.STATIC_CONTEXT, inquirerId, defaultCallMetadata = rpc_metadata_constants_1.DEFAULT_CALLBACK_METADATA) {
        const contextType = 'rpc';
        const { argsLength, paramtypes, getParamsMetadata } = this.getMetadata(instance, methodName, defaultCallMetadata);
    ...
    getMetadata(instance, methodName, defaultCallMetadata) {
        ...
        const getParamsMetadata = (moduleKey) => this.exchangeKeysForValues(keys, metadata, moduleKey, this.rpcParamsFactory);
    ...
    exchangeKeysForValues(keys, metadata, moduleContext, paramsFactory) {
        ...
        return keys.map(key => {
            ...
            if (key.includes(constants_1.CUSTOM_ROUTE_AGRS_METADATA)) {
                ...
                const customExtractValue = this.contextUtils.getCustomFactory(factory, data);
