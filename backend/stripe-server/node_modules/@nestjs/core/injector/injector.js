...
class Injector {
    ...
    async loadInstance(wrapper, collection, module, contextId = constants_2.STATIC_CONTEXT, inquirer) {
        ...
        await this.resolveConstructorParams(wrapper, module, inject, callback, contextId, wrapper, inquirer);
    }
    async resolveConstructorParams(wrapper, module, inject, callback, contextId = constants_2.STATIC_CONTEXT, inquirer, parentInquirer) {
        const inquirerId = this.getInquirerId(inquirer);
        ...
        const resolveParam = async (param, index) => {
            try {
                ...
                const paramWrapper = await this.resolveSingleParam(wrapper, param, { index, dependencies }, module, contextId, inquirer, index);
    ...
    async resolveSingleParam(wrapper, param, dependencyContext, module, contextId = constants_2.STATIC_CONTEXT, inquirer, keyOrIndex) {
        ...
        return this.resolveComponentInstance(module, shared_utils_1.isFunction(token) ? token.name : token, dependencyContext, wrapper, contextId, inquirer, keyOrIndex);
    }
    ...
    async resolveComponentInstance(module, name, dependencyContext, wrapper, contextId = constants_2.STATIC_CONTEXT, inquirer, keyOrIndex) {
        const providers = module.providers;
        const instanceWrapper = await this.lookupComponent(providers, module, Object.assign(Object.assign({}, dependencyContext), { name }), wrapper, contextId, inquirer);
    ...
    async resolveProperties(wrapper, module, inject, contextId = constants_2.STATIC_CONTEXT, inquirer, parentInquirer) {
        ...
        const instances = await Promise.all(properties.map(async (item) => {
            try {
                ...
                const paramWrapper = await this.resolveSingleParam(wrapper, item.name, dependencyContext, module, contextId, inquirer, item.key);
                if (!paramWrapper) {
                    return undefined;
                }
                const inquirerId = this.getInquirerId(inquirer);
